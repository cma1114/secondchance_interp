...
python -u -m mechanistic.collect_contextual_option_lens_diagnostics \
  --config "$CONFIG" \
  --residual-root "$OUT/discovery_original" \
  --baseline-results "$BASE/baseline_results.json" \
  --output "$OUT/analysis/sample_lens_top_tokens.json"

python -u -m mechanistic.analyze_contextual_option_representations \
  --discovery-original "$OUT/discovery_original" \
  --discovery-remapped "$OUT/discovery_remapped" \
