...
residual_root="outputs/mechanistic/qwen36_27b_simplemc_baseline_matched"
baseline_source="$(pwd)/outputs/mechanistic/qwen36_27b_simplemc/shards/baseline"
baseline_link="$residual_root/shards/baseline"
jlens_root="outputs/mechanistic/qwen36_27b_jlens_baseline_matched"
content_root="outputs/mechanistic/qwen36_27b_jlens_baseline_matched_answer_content"
analysis_root="$jlens_root/analysis"

if [[ ! -e "$baseline_link" ]]; then
  ln -s "$baseline_source" "$baseline_link"
fi
...
PYTHONPATH=. python -m mechanistic.jlens_collect \
  --config configs/jlens_qwen36_simplemc_baseline_matched.json \
  --residual-root "$residual_root" \
  --output "$jlens_root" \
  --position-sample 128 \
  --top-k 20 \
