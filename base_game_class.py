...
openrouter_api_key = (
    ...
    or os.environ.get("SPAR_SPRING_2026_OPENROUTER_KEY")
    or os.environ.get("SPAR2025_OPENROUTER_KEY")
)
...
class BaseGameClass:
    ...
    _openrouter_request_interval = 3.5
    _openrouter_provider_only = None
    ...
    def _setup_provider(self):
        ...
        if not self.is_human_player:
            if BaseGameClass._provider_override:
                self.provider = BaseGameClass._provider_override
            elif self.subject_name.startswith("claude"):
    ...
    def _pace_openrouter_request(self):
        """Keep shared-capacity OpenRouter models below their per-minute limit."""
        with self._openrouter_rate_lock:
            elapsed = time.monotonic() - self._openrouter_last_request_at
            wait_seconds = self._openrouter_request_interval - elapsed
            if wait_seconds > 0:
                time.sleep(wait_seconds)
            BaseGameClass._openrouter_last_request_at = time.monotonic()

    def _get_llm_answer(self, options, q_text, message_history, keep_appending=True, setup_text="", MAX_TOKENS=1, temp=0.0, accept_any=True, top_p=None, top_k=None, top_logprobs_count=None):
        """Gets answer from LLM model"""
        # Prepare common data
        # `None` means no setup text. Never stringify it into the model prompt.
        ...
        if options: 
            ...
            system_msg = f"{setup_prefix}Output ONLY the letter of your choice: {options_str}.\n"
        else:
        ...
        requested_top_logprobs = (
            max(len(options), int(top_logprobs_count))
            if top_logprobs_count is not None and options != " "
            else len(options)
        )
        self.last_call_metadata = {
            "top_logprobs_requested": requested_top_logprobs,
            "top_logprobs_returned": None,
                ...
                if self.provider == "Anthropic":
                    ...
                    if self.provider == "OpenRouter":
                        if self.subject_name == "gpt-4.1-2025-04-14": model_name = "openai/gpt-4.1"
                            ...
                            elif self.subject_name.startswith("mimo-"): prefix = 'xiaomi/'
                            ...
                            elif self.subject_name.startswith("gemma-"): prefix = 'google/'
                            elif self.subject_name.startswith("minimax-"): prefix = 'minimax/'
                            elif self.subject_name.startswith("inkling-"): prefix = 'thinkingmachines/'
                            elif self.subject_name.startswith("nemotron-"): prefix = 'nvidia/'
                            else: prefix = ''
                    ...
                    if keep_appending:
                        ...
                        formatted_messages.append(user_msg)
                    self.last_call_metadata["message_roles"] = [
                        message.get("role") for message in formatted_messages
                    ]
                    self.last_call_metadata["system_messages"] = [
                        message.get("content")
                        for message in formatted_messages
                        if message.get("role") == "system"
                    ]
                    if self.provider == "OpenRouter":
                        self._pace_openrouter_request()
                    completion = self.client.chat.completions.create(
                        ...
                        **({"top_logprobs": requested_top_logprobs} if not self.subject_name.startswith("o3") else {}),
                        ...
                        **{'extra_body': {
                            **(
                                ...
                                if (
                                    (
                                        "qwen3.6" in self.subject_name
                                        or "qwen3.5" in self.subject_name
                                        or "gemma-4" in self.subject_name
                                        or "mimo-v2.5" in self.subject_name
                                        or "glm-5" in self.subject_name
                                        or "kimi-k2.6" in self.subject_name
                                        or "deepseek-v3.2" in self.subject_name
                                        or "deepseek-v4" in self.subject_name
                                        or "minimax-m3" in self.subject_name
                                        or "inkling-small" in self.subject_name
                                        or "nemotron-3-super" in self.subject_name
                                        or "_nothink" in self.subject_name
                                        or
                            ...
                            **(
                                ...
                                if (
                                    (
                                        "mimo-v2.5" in self.subject_name
                                        or "glm-5" in self.subject_name
                                        or "kimi-k2.6" in self.subject_name
                                        or "deepseek-v3.2" in self.subject_name
                                        or "deepseek-v4" in self.subject_name
...
#                            'seed': 42,
                            'provider': {
                                **(
                                    {"only": [BaseGameClass._openrouter_provider_only]}
                                    if BaseGameClass._openrouter_provider_only
                                    else {}
                                ),
...
#                                'quantizations': ['fp8'],
                    ...
                    if self.provider == "OpenRouter":
                        ...
                        self.last_call_metadata["serving_provider"] = completion.provider
                        self.last_call_metadata["response_model"] = getattr(completion, "model", None)
                    elif self.provider == "DeepSeek":
                        self.last_call_metadata["serving_provider"] = "DeepSeek API"
                        self.last_call_metadata["response_model"] = getattr(completion, "model", None)
                    ...
                    reasoning_details = getattr(
                        completion.choices[0].message, "reasoning_details", None
                    )
                    self.last_call_metadata["reasoning_present"] = bool(
                        reasoning or reasoning_content or reasoning_details
                    )
                    usage = getattr(completion, "usage", None)
                    self.last_call_metadata["prompt_tokens"] = (
                        getattr(usage, "prompt_tokens", None)
                        if usage is not None
                        else None
                    )
                    self.last_call_metadata["completion_tokens"] = (
                        getattr(usage, "completion_tokens", None)
                        if usage is not None
                        else None
                    )
                    self.last_call_metadata["prompt_cache_hit_tokens"] = (
                        getattr(usage, "prompt_cache_hit_tokens", None)
                        if usage is not None
                        else None
                    )
                    self.last_call_metadata["prompt_cache_miss_tokens"] = (
                    ...
                    self.last_call_metadata["reasoning_tokens"] = (
                        getattr(completion_details, "reasoning_tokens", None)
                        if completion_details is not None
                        else None
                    )
                    self.last_call_metadata["raw_response_content"] = getattr(
                        completion.choices[0].message, "content", None
                    )
                    logprob_content = getattr(
                        getattr(completion.choices[0], "logprobs", None),
                        "content",
                        None,
                    )
                    if logprob_content:
                        self.last_call_metadata["generated_token_trace"] = [
                            token_logprob.token for token_logprob in logprob_content
                        ]
                    if reasoning:
                    ...
                    resp = (response_content or "").strip()
                    ...
                    if len(options) == 1: #short answer, just average
                        ...
                        self.last_call_metadata["top_logprobs_returned"] = len(entry.top_logprobs)
                        ...
                        try:
                            ...
                            if (
                                sum(probability > 0 for probability in probs) < len(options)
                                and callctr < MAX_CALL_ATTEMPTS - 1
                            ):
                                raise ValueError("numerically unusable logprobs")
...
#                            **({"thinking_config": types.ThinkingConfig(thinking_budget=0) if '2.5' in self.subject_name and 'flash' in self.subject_name else {}})
            ...
            try:
                ...
                if (
                    "numerically unusable logprobs" in str(e)
                    or "'NoneType' object has no attribute 'content'" in str(e)
                ):
                    # Some serving shards intermittently replace every non-winning
                    # logprob with the -9999 sentinel or omit the logprob object.
                    # Retry at the original temperature without rate-limit backoff.
                    attempt -= 1
                    continue
                if "429" in str(e) or "503" in str(e) or "not returned" in str(e) or "[Errno 8]" in str(e):
                    ...
                    if getattr(e, "status_code", None) == 429:
                        headers = (getattr(e, "body", None) or {}).get("metadata", {}).get("headers", {})
                        reset_ms = headers.get("X-RateLimit-Reset")
                        if reset_ms:
                            wait_seconds = max(wait_seconds, float(reset_ms) / 1000 - time.time() + 0.5)
                    self._log(f"Retrying after {wait_seconds:.1f} seconds")
