#!/usr/bin/env python3
"""Run the Qwen PopMC Second Chance replication with resumable stages."""
...
N_QUESTIONS = 500
SEED = 42
SOURCE_PATH = Path("PopMC_0_difficulty_filtered.jsonl")
OUTPUT_DIR = Path("outputs/reproduction/popmc")
PREFLIGHT_PATH = OUTPUT_DIR / "preflight.json"
MANIFEST_PATH = OUTPUT_DIR / "stimulus_manifest.json"
STATE_PATH = OUTPUT_DIR / "run_state.json"
COMPILED_PATH = Path("compiled_results_popmc") / f"{MODEL}_phase1_compiled.json"
...
def source_audit() -> dict:
    rows = [json.loads(line) for line in SOURCE_PATH.open(encoding="utf-8") if line.strip()]
    question_counts = collections.Counter(
        str(item.get("question", "")).strip() for item in rows
    )
    return {
        "source_sha256": hashlib.sha256(SOURCE_PATH.read_bytes()).hexdigest(),
        "source_rows": len(rows),
        "unique_question_texts": len(question_counts),
        "duplicate_question_groups": sum(count > 1 for count in question_counts.values()),
        "rows_in_duplicate_groups_excluded": sum(
...
def load_manifest() -> list[dict]:
    ...
    candidates = load_and_format_dataset(DATASET)
    if not candidates or len(candidates) < N_QUESTIONS:
        raise RuntimeError(f"Expected at least {N_QUESTIONS} valid singleton PopMC questions")
    questions = random.Random(SEED).sample(candidates, N_QUESTIONS)
    if len({question["id"] for question in questions}) != N_QUESTIONS:
        raise RuntimeError("Sample contains duplicate question IDs")
    if len({question["question"] for question in questions}) != N_QUESTIONS:
        raise RuntimeError("Sample contains duplicate question texts")
    manifest = {
        "dataset": DATASET,
        "source": str(SOURCE_PATH),
        "seed": SEED,
        "sampling": "simple random sample without replacement from valid singleton question texts",
        "n_questions": N_QUESTIONS,
        "option_assignment": "deterministic per-question SHA-256-seeded shuffle",
        "sampling_frame": {
            **source_audit(),
            "valid_singleton_questions": len(candidates),
        },
        "sample_correct_letter_counts": dict(collections.Counter(
            question["correct_answer"] for question in questions
        )),
        "sample_property_counts": dict(collections.Counter(
            str(question.get("property")) for question in questions
        )),
        "questions": questions,
    }
...
def run_preflight(requested: int, validation_calls: int) -> None:
    ...
    game = BaseGameClass("qwen_popmc_preflight", MODEL, False, "game_logs")
    BaseGameClass._openrouter_request_interval = 3.4
    calls = []
    ...
    output = {
        ...
        "selected_top_logprobs": min(requested, min(returned)),
...
def run_replication() -> None:
    ...
    for stage_name, prompt_variant, correct in [
        ...
        run_second_chance_stage(stage_name, prompt_variant, correct, top_logprobs, state)
    state["complete"] = True
    save_state(state)
    print(f"PopMC replication complete. State: {STATE_PATH}")
