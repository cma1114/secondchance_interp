...
def _messages(config: ExperimentConfig, question: dict, remapped: dict, condition: str):
    messages = build_factorial_messages(question, condition, config.prompt_mode)
...
def collect(
    ...
    invalid_conditions = sorted(set(conditions) - set(FACTORIAL_FEEDBACK))
    if invalid_conditions:
        raise ValueError(f"Unknown factorial conditions: {invalid_conditions}")
    ...
    for condition in conditions:
        ...
        for batch_index, batch_qids in enumerate(
            ...
            for index, (qid, plan_row, remapped, message, prompt) in enumerate(
                ...
                results[qid] = {
                    "question_id": qid,
                    "factorial_condition": condition,
                    "evaluation": "incorrect" if condition.startswith("incorrect_") else "lost",
                    "action": "different" if condition.endswith("_different") else "again",
                    "baseline_original_letter": baseline[qid].get(
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Collect the remapped 2x2 feedback factorial")
    parser.add_argument("--config", type=Path, required=True)
    parser.add_argument("--plan", type=Path, required=True)
    parser.add_argument("--baseline", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path, required=True)
    parser.add_argument(
        "--conditions", nargs="+", choices=sorted(FACTORIAL_FEEDBACK),
        default=sorted(FACTORIAL_FEEDBACK),
    )
    parser.add_argument("--max-questions", type=int)
    args = parser.parse_args()
    collect(
        args.config, args.plan, args.baseline, args.output_dir,
        args.conditions, args.max_questions,
    )
