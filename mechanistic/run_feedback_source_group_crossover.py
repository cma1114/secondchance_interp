...
SOURCE_GROUP_OFFSETS = {
    "feedback_sentence": (0, 1),
    "following_instruction": (2, 3, 4, 5, 6),
    "complete_suffix": tuple(range(7)),
}
SCENARIOS = (
    "natural",
    *(f"{name}_swapped" for name in SOURCE_GROUP_OFFSETS),
)


def selected_group_positions(source_positions: list[int], group: str) -> list[int]:
    if group not in SOURCE_GROUP_OFFSETS:
        raise ValueError(f"Unknown feedback source group: {group}")
    if len(source_positions) != 7:
        raise ValueError("Canonical feedback suffix must contain exactly seven tokens")
    return [source_positions[offset] for offset in SOURCE_GROUP_OFFSETS[group]]
...
def run(args: argparse.Namespace) -> None:
    ...
    for cohort_start in range(0, len(qids), config.batch_size):
        ...
        for pair_start in range(0, len(cohort), 2):
            ...
            for condition_index, condition in enumerate(CONDITIONS):
                for local, qid in enumerate(pair):
                    ...
                    if any(
                        ...
                        raise RuntimeError("Every feedback token must be singular")
                    ...
                    pair_audit["rows"].append({
                        ...
                        "feedback_tokens": decoded,
                        "source_groups": {
                            group: {
                                "positions": selected_group_positions(feedback_positions, group),
                                "tokens": [decoded[offset] for offset in offsets],
                            }
                            for group, offsets in SOURCE_GROUP_OFFSETS.items()
                        },
                    })
            ...
            for scenario in SCENARIOS:
                ...
                try:
                    if scenario != "natural":
                        group = scenario.removesuffix("_swapped")
                        selected_by_row = {
                            row: selected_group_positions(row_sources[row], group)
                            for row in range(4)
                        }
                        ordinary = BatchedSDPADownstreamSourceKVPatcher(
                            parts,
                            {
                                row: (donors[row], selected_by_row[row], ordinary_layers)
                                for row in range(4)
                            },
                        )
        ...
        print(
            f"feedback source groups: {int(arrays['completed'].sum())}/{len(qids)}; "
    ...
    metadata = {
        "experiment": "grouped causal crossover of the feedback sentence and following instruction",
        "questions": len(qids),
        "conditions": list(CONDITIONS),
        "scenarios": list(SCENARIOS),
        "source_group_offsets": {key: list(value) for key, value in SOURCE_GROUP_OFFSETS.items()},
        ...
        "source_scope": (
            "Reciprocal downstream ordinary-attention K/V and complete GLA k/v/g/beta "
            "write crossover at every applicable layer, preserving source-token outputs."
        ),
