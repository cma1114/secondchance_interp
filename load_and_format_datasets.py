...
def load_and_format_dataset(dataset_name, num_questions_needed=None, split=None, skip_questions=None):
    ...
    if dataset_name=="GPSA":
        ...
        raise ValueError(f"Unknown dataset name: {dataset_name}. Supported datasets include GPQA, MMLU, TruthfulQA, SimpleMC, TriviaMC, and PopMC.")
...
def load_and_format_triviamc(num_questions_needed=None, split="test", skip_questions=None):
    """Load the fixed local TriviaMC evaluation set without relabeling options."""
    import json

    filename = "./TriviaMC_difficulty_filtered.jsonl"
    print(f"Attempting to load TriviaMC from {filename}...")
    try:
    ...
    if num_questions_needed is None:
        num_questions_needed = len(dataset)

    formatted_questions = []
    question_ids_added = set()
    question_texts_added = set()
    for item in dataset:
        if len(formatted_questions) >= num_questions_needed:
            break
        ...
        if (
            ...
            or list(options.keys()) != ["A", "B", "C", "D"]
            or correct_answer not in options
            or any(not str(value).strip() for value in options.values())
            or len({str(value).strip() for value in options.values()}) != 4
        ):
            continue
        if qid in question_ids_added or question_text in question_texts_added:
            continue
        if skip_questions is not None and question_text in skip_questions:
            continue
        ...
        question_ids_added.add(qid)
        question_texts_added.add(question_text)

    if len(formatted_questions) < num_questions_needed:
        print(
            f"Warning: Only able to format {len(formatted_questions)} unique questions, "
            f"but {num_questions_needed} were requested."
        )
    print(f"Successfully formatted {len(formatted_questions)} TriviaMC questions.")
    return formatted_questions


def load_and_format_popmc(num_questions_needed=None, split="test", skip_questions=None):
    """Load structurally valid singleton PopMC questions with stable option order."""
    import collections
    import json

    filename = "./PopMC_0_difficulty_filtered.jsonl"
    print(f"Attempting to load PopMC from {filename}...")
    try:
        with open(filename, "r", encoding="utf-8") as f:
            dataset = [json.loads(line) for line in f if line.strip()]
    except Exception as e:
    ...
    question_counts = collections.Counter(
        str(item.get("question", "")).strip() for item in dataset
    )
    formatted_questions = []
    for item in dataset:
        question_text = str(item.get("question", "")).strip()
        if not question_text or question_counts[question_text] != 1:
            continue
        if skip_questions is not None and question_text in skip_questions:
            continue
        qid = str(item.get("qid", "")).strip()
        correct_text = str(item.get("correct_answer", "")).strip()
        ...
        if (
            ...
            or any(not str(value).strip() for value in distractors)
        ):
            continue
        answer_texts = [correct_text] + [str(value).strip() for value in distractors]
        if len({value.casefold() for value in answer_texts}) != 4:
            continue

        # Stable per-item shuffling avoids dependence on global RNG state.
        option_rng = random.Random(
            int(hashlib.sha256(f"popmc-options:{qid}".encode("utf-8")).hexdigest()[:16], 16)
        )
        option_rng.shuffle(answer_texts)
        options = {letter: answer for letter, answer in zip("ABCD", answer_texts)}
        correct_label = next(
            letter for letter, answer in options.items() if answer == correct_text
        )
        formatted_questions.append({
            "id": qid,
            "question": question_text,
            "options": options,
            "correct_answer": correct_label,
            "property": item.get("prop"),
            "subject_popularity": item.get("s_pop"),
            "object_popularity": item.get("o_pop"),
        })

    if num_questions_needed is not None:
        formatted_questions = formatted_questions[:num_questions_needed]
    print(
        f"Successfully formatted {len(formatted_questions)} singleton PopMC questions."
    )
    return formatted_questions
